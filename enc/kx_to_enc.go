...
// NewKXEncKey creates new parsed encryption key, which may be used to create encryptors
// from any key exchange scheme.
//
// Note: It uses ephemerical keypair for each encryption, so enc key options passed may include nonce counter
// to be always used in favour of random nonces.
func NewKXEncKey(
	kxGen kx.Gen,
	pkx kx.Pub,
	kxp kx.Parser,
...
) (enc EncKey, err error) {
	enc = EncKeyFunc(func(options interface{}) (enc Encryptor, err error) {
		// 1. Generate ephemeric KX keypair and process it
		rawPK, rawSK, err := kxGen.GenKX(options, nil, nil)
		if err != nil {
			return
		}
...
func NewKXDecKey(
	...
	dec = DecKeyFunc(func(options interface{}) (dec Decryptor, err error) {
